...
public function store(Request $request, JobOffer $jobOffer)
{
    ...
    // Get resumes for a specific job offer (only for job creator)
    public function index(JobOffer $jobOffer)
    {

        $resumes = $jobOffer->resumes()->with('user', 'jobOffer')->get();
    ...
    public function getUserApplications()
    {
        $applications = Resume::where('user_id', auth()->id())
            ->with(['jobOffer', 'jobOffer.company']) // eager load relationships
            ->get()
            ->map(function ($resume) {
                return [
